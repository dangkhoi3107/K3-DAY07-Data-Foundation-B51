...
| `2A202601787-NguyenDangDuc/` | Nguyễn Đăng Đức | ⚠️ chưa xác nhận | ⬜ chưa có | Code viết xong nhưng khi Đức tự chạy `pytest` trên nhánh gốc của mình thì vẫn fail vì đặt sai vị trí (chi tiết: `THAM_KHAO_Duc.md`). Số liệu retrieval trong report cũng chưa khớp corpus — cần Đức tự xác nhận lại |
| `2A202601051-DoTuanSon/` | Đỗ Tuấn Sơn | ⚠️ chưa xác nhận | ⬜ chưa có | Có `HeadingChunker` tự viết đúng yêu cầu bắt buộc K3 (thiết kế tốt), nhưng cùng lỗi vị trí code như Đức nên chưa tự pass được 42/42 (chi tiết: `THAM_KHAO_Son.md`) |
| `2A202601269-TranDucBaoTrung/` | Trần Đức Bảo Trung | ✅ 42/42 | ✅ `RecursiveChunker(420)` + tự viết `LightweightVietnameseEmbedder` (sklearn `HashingVectorizer`) | Điểm truy xuất 10/10 — cao nhất nhóm |
